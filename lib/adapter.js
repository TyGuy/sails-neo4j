...
var adapter = module.exports = (function() {
  ...
  function query(connection, collection, q, params, cb) {
  ...
  function getRelatedNodesWithDirection(connection, collection, predecessorParams,
    ...
    if (successorParams) {
      orders = successorParams.sort
      if (successorParams.sort) { delete successorParams.sort }
    }

    const queryBuilder = new QueryBuilder()

    /// build matches
    const predLabels = collection ? [collection] : []
    const succLabels = successorCollectionName ? [successorCollectionName] : []
    const relLabels = relationshipType ? [relationshipType] : []

    const predNodeMatch = { type: 'node', labels: predLabels, ref: 'pred' }
    const succNodeMatch = { type: 'node', labels: succLabels, ref: 'n' }
    const relMatch = { type: 'rel', ref: 'r', labels: relLabels }
    if (directed) { relMatch.direction = directed }

    queryBuilder.match([predNodeMatch, relMatch, succNodeMatch])

    // build wheres
    queryBuilder.where({
      pred: predecessorParams || {},
      n: successorParams || {},
      r: relationshipParams || {}
    })

    // build returns
    queryBuilder.returns('n')
    ...
    query(connection, collection, builtQuery.query, builtQuery.params, cb);
  }

  // function builtQueryForLinkedNodes(collection, predecessorParams, successorCollectionName, successorParams,
  //   relationshipType, relationshipParams, directed, returns) {
  //
  // }
  ...
  return {
    ...
    destroy: function(connection, collection, params, cb) {
      // morph the object into the thing we want:
      // if this is an object with keys but that does not include "where",
      // then add the "where" key
      if (params && typeof params === 'object' && Object.keys(params).length > 0 && !params.where) {
        params = { where: params }
      }
