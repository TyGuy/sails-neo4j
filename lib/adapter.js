...
var adapter = module.exports = (function() {
  ...
  function query(connection, collection, q, params, cb, unique) {
    const props = utils.encodeProps(params)

    connections[connection].graph(function(gr) {
      gr.cypher({ query: q.join('\n'), params: props }, (err, results) => {
        if (connectionSettings.debug) console.log(q.join('\n'), results);
        if (err) { return cb(err, null) }

        var data = [];
        for(var i = 0; i < results.length; i++) {
          var resultNode = results[i].n

          // TODO: change this!
          // Eventually: we want it to use an id we assign it
          // (but maybe still default to neo4j internal).
          // For Now: we will use internal id so we don't break our app.
          resultNode.id = resultNode._id

          var decodedProps = utils.decodeProps(resultNode.properties)

          var obj = Object.assign({}, resultNode, { data: decodedProps })
  ...
  return {
    ...
    find: function(connection, collection, params, cb) {
      ...
      q = utils.addOrderClause(q, params, 'n')
    ...
    update: function(connection, collection, params, values, cb) {
      ...
      q = [
        ...
        'SET ' + andJoin('n', _.omit(values, 'id'), ',', newNamespace),
        'RETURN n'
      ];
    ...
    updateLink: function(connection, collection, fromParams, toCollection, toParams, relData, cb) {
      ...
      q.push('SET ' + andJoin(relName, _.omit(relUpdateParams, 'id'), ',', relUpdateNamespace))
