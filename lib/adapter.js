...
var adapter = module.exports = (function() {
      ...
      gr.query(q.join('\n'), params, function (err, results) {
        ...
        if (err) {
          ...
          for(i=0; i<results.length;i++){
            var obj = Object.assign({}, results[i].n);
            if (results[i].labels) { obj.labels = results[i].labels }
  ...
  return {
    ...
    // TODO: this is shitty, because it's all duplicated and it requires the return value to be 'n'.
    ...
    getRelatedNodes: function(connection, collection, predecessorParams, successorCollectionName, successorParams, relationshipType, relationshipParams, cb) {
      ...
      const hasSuccessorParams = successorParams && Object.keys(successorParams).length > 0

      let successorClause = ''
      if (successorCollectionName) {
        successorClause = successorClause + 'AND ' + successorCollectionName + ' '
      }
      if(hasSuccessorParams) {
        successorClause = successorClause + 'AND ' + toWhere('n', { where: successorParams }, successorNamespace)
      }
      ...
      if (successorClause.length > 0) {
        q.push(successorClause)
      }

      q.push('RETURN n, labels(n) as labels')
    ...
    link: function(connection, collection, predecessorParams, successorCollectionName, successorParams, relationshipType, relationshipParams, cb) {
      ...
      q = [
        ...
        'MERGE (a)-[n:' + relationshipType + relationshipParams + ']->(b)',
